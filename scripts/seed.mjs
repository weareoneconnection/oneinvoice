...
async function main() {
  // Create admin user if not exists (no upsert — HTTP mode doesn't support transactions)
  const existing = await prisma.user.findUnique({ where: { email: 'admin@demo.com' } });
  if (!existing) {
    const passwordHash = await bcrypt.hash('demo1234', 10);
    await prisma.user.create({ data: { email: 'admin@demo.com', passwordHash, name: 'Admin' } });
